...
- **Pop out** — the `POP OUT` button in the viewer shrinks the whole app into a small window that floats on top of other apps, and drops the fullscreen app to the background. It is draggable (by its title bar) and resizable (by the corner grip), it reshapes itself to each item's aspect ratio as you swipe so media fills it instead of sitting in letterbox bars (keeping the footprint you last dragged it to), and it is the *whole* app: browse folders, open media, swipe, zoom, split and play video inside it. `EXPAND` brings the fullscreen app back showing whatever the mini window was on; `✕` closes it. `SPLIT` works there too — the window widens to hold both panes. Position and size are remembered, as is a separate tile size for the small window. Needs the **Display over other apps** permission (the app asks the first time). Rename, filter, help and the updater are left out of the mini window — they need a keyboard or their own dialog window, which an overlay can't host.
